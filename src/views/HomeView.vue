...
function showSystemMessage(name, stype) {
  ...
  const message_text = stype === 'welcome' ? `Bienvenido ${name}` : stype === 'disconnect' ? `Estas desconectado del servidor` : `${name} se ha ${stype === 'join' ? 'unido' : 'salido'} `
...
<template>
  <div class="container">
    ...
    <div class="input">
      <input ref="inputRef" @keyup.enter="handleSend" placeholder="Preguntale a Cannabito" class="input-text" type="text" v-model="message" />
      <button :disabled="!message || isAIProcessing" @click="handleSend" class="button">Enviar</button>
    </div>
    <div class="footer">
      <div class="toggle">
